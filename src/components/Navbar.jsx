  ...
  return (
    <nav className="container flex items-center relative mx-auto px-5 sm:px-2 py-6 sm:py-[2.125rem] z-10">
      ...
      <div className="hidden sm:flex flex-1 justify-end">
        <ul className="flex items-center gap-12">
          {links.map((link) => (
            <li
              key={link.id}
              className="font-sans font-semibold text-lg text-neutral-white"
            >
              <a href={`#${link.title}`}>{link.title}</a>
            </li>
          ))}
          <button
            className="font-serif font-black uppercase text-neutral-blue-desaturated py-4 px-8 bg-neutral-white rounded-full cursor-pointer hover:text-neutral-white hover:bg-opacity-30"
            type="button"
          >
            Contact
          </button>
      ...
      <div className="sm:hidden">
        <button type="button" onClick={() => setToggleMenu((prev) => !prev)}>
        ...
        >
          <ul className="flex flex-col items-center">
            ...
            <button
              className="font-serif font-black uppercase text-neutral-blue-desaturated py-4 px-8 bg-primary-yellow rounded-full cursor-pointer"
              type="button"
            >
              Contact
            </button>
